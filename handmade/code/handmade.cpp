...
MoveEntity(game_state *GameState, entity Entity, real32 dt, move_spec *MoveSpec, v2 ddP)
{
    ...
    // a = a <-- d-pad
    // TODO: ODE here!
    ddP += -MoveSpec->Drag * Entity.High->dP; // decelerate by the velocity he's moving (friction)
    ...
    /*
      ...
      uint32 EntityTileWidth = CeilReal32ToInt32(Entity->Width / TileMap->TileSideInMeters); // An entity's width in tiles. Probably going to be 1
...
UpdateSword(game_state *GameState, entity Entity, real32 dt)
{
    ...
    if(Entity.Low->DistanceRemaining < 0.0f)
    {
        ChangeEntityLocation(&GameState->WorldArena, GameState->World,
                             Entity.High->LowEntityIndex, Entity.Low,
                             &Entity.Low->P, 0);
    }
}
...
DrawHitpoints(low_entity *LowEntity, entity_visible_piece_group *PieceGroup)
{
    if(LowEntity->HitPointMax >= 1)
    {
        ...
        v2 HitP = V2((LowEntity->HitPointMax - 1) * (-0.5f * SpacingX), -0.25f);
