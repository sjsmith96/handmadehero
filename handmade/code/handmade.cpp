...
AddPlayer(game_state *GameState)
{
    ...
    EntityLow->Width = 1.0f; //0.75f * Entity->Height;
...
AddWall(game_state *GameState, int32 AbsTileX, int32 AbsTileY, int32 AbsTileZ)
{
    ...
    EntityLow->P = ChunkPositionFromTilePositon(GameState->World, AbsTileX, AbsTileY, AbsTileZ); 
...
#if 0
    for(int32 RelRow = -100;
        RelRow < 10;
        ++RelRow)
    {
        for(int32 RelColumn = -200;
            RelColumn < 20;
            ++RelColumn)
        {
