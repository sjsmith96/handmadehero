           ...
           real32 cAlpha = 1.0f)
{
            ...
            Y++)
    {
            ...
            X++)
        {
            ...
            *Dest = (((uint32)(R + 0.5f) << 16) | /* "or" the RBG together with truncation */
                     ((uint32)(G + 0.5f) << 8) |
                     ((uint32)(B + 0.5f) << 0));
...
AddStair(game_state *GameState, int32 AbsTileX, int32 AbsTileY, int32 AbsTileZ)
{
    ...
    Entity.Low->Sim.Dim.Y = 2 *GameState->World->TileSideInMeters;    
...
AddPlayer(game_state *GameState)
{
    ...
    Entity.Low->Sim.Dim.X = 1.0f; //0.75f * Entity->Height;
    AddFlags(&Entity.Low->Sim, (EntityFlag_Collides |
                                EntityFlag_Moveable));
            ...
            )
        {
               ...
               ((*Rule)->StorageIndexB == StorageIndex))
            {
                ...
                *Rule = (*Rule)->NextInHash;
                ...
                RemovedRule->NextInHash = GameState->FirstFreeCollisonRule;
                GameState->FirstFreeCollisonRule = RemovedRule;
...
AddCollisionRule(game_state *GameState, uint32 StorageIndexA, uint32 StorageIndexB, bool32 CanCollide)
{
    ...
    if(!Found)
    {
        Found = GameState->FirstFreeCollisonRule;
        if(Found)
        {
            GameState->FirstFreeCollisonRule = Found->NextInHash;
        }
        else
        {
...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
    ...
    if(!Memory->IsInitialized)
    {
        ...
        NewCameraP = ChunkPositionFromTilePositon(GameState->World,
                                                  CameraTileX,
                                                  CameraTileY,
                                                  CameraTileZ);
        ...
        for(int FamiliarIndex = 0;
            ...
            ++FamiliarIndex)
        {
            uint32 FamiliarOffsetX = (RandomNumberTable[RandomNumberIndex++] % 10) - 5;
            uint32 FamiliarOffsetY = (RandomNumberTable[RandomNumberIndex++] % 10) - 5;
    ...
    DrawRectangle(Buffer,
                  V2(0.0f, 0.0f), V2( (real32) Buffer->Width, (real32) Buffer->Height),
                  0.5f, 0.5f, 0.5f);
#else
        ...
        if(Entity->Updatable)
        {
            ...
            switch(Entity->Type)
            {
                ...
                case EntityType_Familiar:
                {
                    ...
                    PushBitmap(&PieceGroup, &HeroBitmaps->Head, V2(0, 0), 0.5f * BobSin, HeroBitmaps->Align);
                    break;
                }
